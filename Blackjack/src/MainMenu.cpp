...
void MainMenu::PrintTitle() const
{
	...
	XY(21, 19);
	WriteLine(Center("BY", width));
	SetColor({7, 130, 73});
	WriteLine(Center("Kendric M. Ordoñez", width));
	WriteLine(Center("Shawn Aaron B. Quirante", width));
	SetColor({252, 189, 20});
	WriteLine(Center("FEUTech BSCSSE Freshmen", width));
	ResetColor();
}
